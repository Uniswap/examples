...
const Example = () => {
  ...
  const onIncreasePosition = useCallback(
    async (positionId: number[] | undefined) => {
      // todo handle multiple positions
      if (!positionId) {
        return
      }
      setTxState(TransactionState.Sending)
      setTxState(await modifyPosition(positionId))
    },
    []
  )
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <button
          onClick={() => onIncreasePosition(positionIds)}
          disabled={
            txState === TransactionState.Sending ||
            getProvider() === null ||
            CurrentConfig.rpc.mainnet === '' ||
